...
import { MessageCircle, Send, ArrowLeft, Hotel, User, Home } from "lucide-react";
import { getConversations, getMessages, sendMessage, markConversationRead, getAllHotels, createNotification } from "../../lib/api";
...
export default function CustomerMessages() {
  ...
  useEffect(() => {
    loadConversations();
    loadHotels();
    // Poll every 10s for new messages/conversations
  ...
  useEffect(() => {
    // If hotelId is in URL, auto-start chat with that hotel
    const hotelId = searchParams.get("hotelId");
    if (hotelId && hotels.length > 0) {
      const hotel = hotels.find((h) => h.id === hotelId);
      if (hotel) {
        startNewConversation(hotel);
      }
    }
  }, [searchParams, hotels]);
  ...
  const startNewConversation = (hotel: any) => {
    if (!user) return;
    const conversationId = `${user.email}_${hotel.contact.email}`;
    const newConv: Conversation = {
      ...
      hotelEmail: hotel.contact.email,
      lastMessage: "",
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const messageObj: Message = {
      ...
      receiverEmail: selectedConv.hotelEmail,
    };
    ...
    createNotification({
      targetEmail: selectedConv.hotelEmail,
      title: `New message from ${user.name}`,
      message: newMessage.trim(),
      type: "message",
      linkPath: "/owner/messages",
    });
    loadConversations();
  };
            ...
            <div className="w-1/3 border-r border-gray-200 flex flex-col">
              ...
              {showNewChat && (
                <div className="p-4 bg-blue-50 border-b">
                  <p className="text-sm font-semibold text-gray-700 mb-2">Select a hotel:</p>
                  <div className="space-y-1 max-h-60 overflow-y-auto">
                    {hotels.map((hotel) => (
                      <button
                        key={hotel.id}
                        onClick={() => startNewConversation(hotel)}
                        className="w-full text-left px-3 py-2 rounded-lg hover:bg-blue-100 transition text-sm"
                      >
                        <p className="font-medium text-gray-800">{hotel.name}</p>
                        <p className="text-xs text-gray-500">{hotel.location?.address}</p>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {conversations.length === 0 && !showNewChat && (
                  <div className="flex flex-col items-center justify-center h-full text-gray-400 px-4 text-center">
                    ...
                    <p className="text-xs mt-1">Start a new chat with a hotel</p>
                  </div>
                )}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="size-10 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">
                        <Hotel className="size-5 text-green-600" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <p className="font-semibold text-gray-800 text-sm">{conv.hotelName}</p>
                          {conv.unreadCount > 0 && (
                        ...
                        <p className="text-xs text-gray-500 truncate mt-0.5">{conv.lastMessage || "Start a conversation"}</p>
              ...
              {selectedConv ? (
                <>
                  ...
                  <div className="bg-gray-50 border-b px-6 py-4 flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="font-bold text-gray-800">{selectedConv.hotelName}</h2>
